...
{
    public class PhysicSimulationInAir
    {
        #region info
        public String Author = "Timo Schmetzer";
        public Version Version = new Version(0, 6);
        public String Comment = "Numerical Simulation assuming Ball always in Air";
        #endregion
        ...
        public static void RunSimulation(IPhysicsState Istate, double elapsedSeconds)
        {
            ...
            if (tcallx >= tcally)
            {
                ...
                Physics.PhysicsInAir.CalcPhysics(state, tcally);
                state.PlateVelocity = new Vector(signx*Istate.PlateVelocity.X, 0);
                Physics.PhysicsInAir.CalcPhysics(state, tcallx - tcally);
                state.PlateVelocity = new Vector(0, 0);
                Physics.PhysicsInAir.CalcPhysics(state, elapsedSeconds - tcallx);
            }
            else
            {
                state.PlateVelocity = new Vector(signx * Istate.PlateVelocity.X, signy * Istate.PlateVelocity.Y);
                Physics.PhysicsInAir.CalcPhysics(state, tcallx);
                state.PlateVelocity = new Vector(0, signy * Istate.PlateVelocity.Y);
                Physics.PhysicsInAir.CalcPhysics(state, tcally - tcallx);
                state.PlateVelocity = new Vector(0, 0);
                Physics.PhysicsInAir.CalcPhysics(state, elapsedSeconds - tcally);
            }
            #endregion
