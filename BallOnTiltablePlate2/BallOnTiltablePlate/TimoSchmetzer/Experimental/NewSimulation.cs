...
namespace BallOnTiltablePlate.TimoSchmetzer
{
    ...
    {
        public static void RunSimulation(PhysicsCalculator Calculator, IPhysicsState Istate, double elapsedSeconds)
        {
            ...
            if (tcallx >= tcally)
            {
                ...
                Calculator.CalcPhysics(state, tcally);
                state.PlateVelocity = new Vector(signx*Istate.PlateVelocity.X, 0);
                Calculator.CalcPhysics(state, tcallx - tcally);
                state.PlateVelocity = new Vector(0, 0);
                Calculator.CalcPhysics(state, elapsedSeconds - tcallx);
            }
            else
            {
                state.PlateVelocity = new Vector(signx * Istate.PlateVelocity.X, signy * Istate.PlateVelocity.Y);
                Calculator.CalcPhysics(state, tcallx);
                state.PlateVelocity = new Vector(0, signy * Istate.PlateVelocity.Y);
                Calculator.CalcPhysics(state, tcally - tcallx);
                state.PlateVelocity = new Vector(0, 0);
                Calculator.CalcPhysics(state, elapsedSeconds - tcally);
            }
            #endregion
